...
import QuestModel, { QUEST_TARGET_ATTRIBUTES, QUEST_TYPES, type QuestScalingDirection, type QuestTargetAttribute, type QuestType } from "@/models/Quest";
import DailyQuestCompletionModel from "@/models/DailyQuestCompletion";
import ShopItemModel from "@/models/ShopItem";
import UserModel, { PLAYER_ATTRIBUTES, type PlayerAttribute, type PlayerRank, type UnlockedRankReward } from "@/models/User";
import { dateKeyToUtcDate, getCurrentGameDateKey, processDailyQuestRollover } from "@/lib/dailyQuestRollover";
...
function formNumber(formData: FormData, name: string): number {
  ...
  if (!value || !Number.isFinite(parsed) || parsed < 0) throw new Error(`Invalid ${name}.`);
  return parsed;
}
...
function parseSelectedAttribute(formData: FormData, allowedAttributes: PlayerAttribute[]): PlayerAttribute {
  const value = formString(formData, "selectedAttribute");
  if (!value || !PLAYER_ATTRIBUTES.includes(value as PlayerAttribute)) throw new Error("A valid reward attribute must be selected.");
  const attribute = value as PlayerAttribute;
  if (!allowedAttributes.includes(attribute)) throw new Error("The selected attribute is not valid for this quest.");
  return attribute;
}
...
function applyXpAndLevelUps(user: { currentXp: number; xpToNextLevel: number; level: number; rank: PlayerRank; unlockedRankRewards: RankRewardRecord[] }, xpReward: number): void {
  ...
  while (user.currentXp >= user.xpToNextLevel) {
    ...
    if (reward && !user.unlockedRankRewards.some((unlockedReward) => unlockedReward.rank === newRank)) {
      user.unlockedRankRewards.push({ ...reward, unlockedAt: new Date() });
    }
  }
}
...
export async function createQuest(formData: FormData): Promise<void> {
  ...
  if (!title || !description) throw new Error("Quest title and description are required.");
  if (!QUEST_TYPES.includes(type)) throw new Error("Invalid quest type.");
  if (!QUEST_TARGET_ATTRIBUTES.includes(targetAttribute)) throw new Error("Invalid quest target attribute.");

  await QuestModel.create({ title, description, type, targetAttributes: [targetAttribute], xpReward, goldReward, completed: false, isPermanentDaily: false, scalingDirection: "CONSTANT" satisfies QuestScalingDirection });
  revalidatePath("/");
...
export async function createShopItem(formData: FormData): Promise<void> {
  ...
  if (!title || !description) throw new Error("Shop item title and description are required.");
  await ShopItemModel.create({ title, description, cost, ...(stock === undefined ? {} : { stock }) });
  revalidatePath("/");
  revalidatePath("/admin");
}
...
export async function completeQuest(questId: string, formData: FormData): Promise<void> {
  ...
  if (quest.isPermanentDaily && quest.type === "DAILY") {
    ...
    const dailyRecord = await DailyQuestCompletionModel.findOne({ userId: currentUser._id, dailyQuestKey: quest.dailyQuestKey, date });
    ...
    const selectableAttributes = quest.targetAttributes.filter((attribute): attribute is PlayerAttribute => attribute !== "NONE" && PLAYER_ATTRIBUTES.includes(attribute));
    if (selectableAttributes.length === 0) throw new Error("This quest does not have a valid reward attribute.");
    const allottedAttribute = selectableAttributes.length === 1 ? selectableAttributes[0] : parseSelectedAttribute(formData, selectableAttributes);
